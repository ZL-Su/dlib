...
// ----------------------------------------------------------------------------------------
	 ...
	 public:
		 ...
		 loss_multimulticlass_log_(const std::map<std::string, labels_type>& labels)
		 {
			 for (auto& l : labels)
			 {
		 ...
		 std::map<std::string, labels_type> get_labels() const
		 {
			 std::map<std::string, labels_type> info;
		 ...
		 double compute_loss_value_and_gradient(const tensor& input_tensor,
			 const_label_iterator truth_begin, SUBNET& sub) const
		 {
			 const tensor& output_tensor = sub.get_output();
			 tensor& grad = sub.get_gradient_input();
